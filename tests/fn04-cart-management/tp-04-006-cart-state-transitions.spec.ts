...
/**
 * TP-04-006 — Verify cart state transitions from empty through populated
 * and back to empty, with totals recalculated on each change. Covers
 * TC-04-006 (#1 to #9).
 *
 ...
 * (e.g. a Cloudflare interstitial) breaks the test instead of DEF-F4-01/02.
 *
 * Intercase dependency: TP-04-002's valid quantity acceptance step.
 */
test.describe('FN-04 Cart Management', () => {
  test('TP-04-006 cart state transitions', async ({ page }, testInfo) => {
    ...
    await test.step('TC-04-006 #2 — add product B, a distinct product (line count 2)', async () => {
      await product.goto(PRODUCT_HANDLES.stripedTop);
      const resp = page.waitForResponse((r) => r.url().includes('/cart/add'), { timeout: 10_000 }).catch(() => null);
      await product.addToCartButton.click();
      await resp;
