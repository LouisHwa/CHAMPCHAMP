...
Local working note (`docs/` is gitignored). Send this file to whoever is
implementing account-related tests.
...
7. In a **PowerShell** terminal at the repo root (not Git Bash —
   ...
   node scripts/cookies-to-storage-state.mjs cookie-header.txt
   node scripts/verify-auth-session.mjs
